...
if (isset($_GET['method'])) {
    switch ($_GET['method']) {
        case "plans":
            $myURL = "/subscription/plans";
            $exempt = true;
            break;
        case "products":
            $myURL = "/product/get_products";
            $exempt = true;
            break;
        default:
            $myURL = "/product/generate_pay_link";
            $exempt = false;
            break;
    }
} else {
...
try {
    if ($errMsg) {
        ...
        curl_setopt($ch, CURLOPT_URL,"https://vendors.paddle.com/api/2.0" . $myURL);
